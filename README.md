...
두 가지는 로그가 **주장하지 않는** 것이고, 스키마가 그렇게 말하도록 만들었습니다:

- **`detected_at`이지 `changed_at`이 아닙니다.** 시스템이 처음 본 시각만 알 수
  있습니다. 토요일에 고치고 월요일에 돌리면 월요일로 찍힙니다.
- **사람 actor는 추정, AI actor는 사실입니다.** 설정 변경은 그 변경을 처음
  발견한 프로세스의 OS 사용자(`user@host`)로 기록합니다 — 1인 머신에서는 대개
  맞지만 **보안적 주장이 아닙니다.** 이 대시보드에는 인증이 없습니다. AI 보류는
  그 검토가 직접 쓴 것이라 정확합니다.
