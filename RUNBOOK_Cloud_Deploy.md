...
> 쓰던 10:00에 맞췄다.
>
> ✅ **`m7-trade-schedule`, 매일 10:10 KST (2026-09-02).** 킬 스위치(6장)가 붙은 뒤라 조건이
> 충족됐다. 10:10은 임의의 시각이 아니다 — 리스크 게이트는 장이 닫혀 있으면 `order-hours-closed`로
> 전량 거부하는데, 토스 캘린더의 `dayMarket`(미국 주간거래)이 이 시각에 열려 있다.
>
...
- [ ] **첫 월요일 PAPER 실행 확인 (9/7)** — momentum-DCA는 월요일에만 신호를 낸다. 게이트→발주
      경로가 실데이터로 도는 첫 날이고, 미해결 #6(매수가능금액이 USD로 잡히는가)이 드러날 자리다

### 완료

- [x] 자동 빌드 (7장) · `m7-daily` Job (2장) · 스케줄 (3장) · 고정 IP/NAT (0-7)
- [x] 시크릿·버킷·서비스계정 (0-4~0-6) · 대시보드 배포 + IAP (6장)
- [x] `m7-trade` Job 정상화 + 매일 10:10 스케줄 (2·3장, 2026-09-02)
